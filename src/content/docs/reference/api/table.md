...
----------------|---------|------------------------------------------
 ...
 `visible`      | `TRUE`/`FALSE` or string | whether the column should be visible. if a string is specified, this must be a data-binding to an option.
...
> [!IMPORTANT]
> You must explicitly name the `rowKey` argument: `setRow(rowKey=...)` to differentiate from `setRow(rowNo=...)`.
...
> [!IMPORTANT]
> You must explicitly name the `rowNo` argument when calling this method: `setRow(rowNo=...)` to differentiate from `setRow(rowKey=...)`.
