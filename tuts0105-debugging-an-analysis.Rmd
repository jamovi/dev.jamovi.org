...
We will start with the common scenario in which an error message is presented due to a user's lack of inputs. Start by removing both `len` from the `Dependent Variable` and also remove `supp` from the `Grouping Variable`. The variables should be removed from these sections and look like this:

![](tuts0105-debugging-an-analysis-move-vars.png){width="400"}

Now, you will see an error message where the output previously appeared:

![](tuts0105-debugging-an-analysis-user-end-error-message.png){width="600"}
